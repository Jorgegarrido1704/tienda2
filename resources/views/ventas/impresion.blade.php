...
<head>
    ...
    <style>
        ...
        .datos {
            ...
            margin-bottom: 10px;
            /* Ajusta el espacio entre bloques */
            font-size: x-large;
        }
        ...
        .datos div {
            width: 48%;
            /* Ancho del 48% para que haya dos divs en una fila */
            box-sizing: border-box;
            /* Incluye el relleno y el borde en el ancho total */
            margin-bottom: 10px;
            /* Ajusta el espacio entre divs */
            font-size: x-large;
        }
...
<body>
    ...
    <div class="datos-linea">
        <div>
            <h1>Promotor: {{ $venta['promotor'] ?? 'N/A' }}</h1>
        </div>
        <div>
            <h1>Vendedor: {{ $venta['vendedor'] ?? 'N/A' }}</h1>
        </div>
        <div>
            <h1>Cobrador: {{ $venta['cobrador'] ?? 'N/A' }}</h1>
        </div>
    </div>
    <div>
            ...
            <tbody>
                ...
                @for ($i = 1; $i < count($articulos); $i++)
                    <tr class="art">
                        ...
                        <td class="art">
                            <h1>{{ $articulos[$i] ?? 'N/A' }}</h1>
                        </td>
                        <td class="art">
                            <h1>{{ intval($venta['pre'] / (count($articulos) - 1)) ?? 1 }}</h1>
                        </td>
                    </tr>
                @endfor
    ...
    <div class="datos">
        ...
        <h1>Fecha de vencimiento:
            {{ date('d-m-Y', strtotime($venta['date'] . ' + ' . $venta['plazo'] . ' months')) ?? 'N/A' }}</h1>
        <table>
            <thead>
                <th class="cob">
                    <h2>Fecha</h2>
